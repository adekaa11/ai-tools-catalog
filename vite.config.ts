  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/manus-storage", async (req, res) => {
        ...
        if (!forgeBaseUrl || !forgeKey) { res.writeHead(500); res.end("Proxy error"); return; }
        try {
...
const customResolvePlugin = (): Plugin => ({
  name: "fix-legacy-paths",
  resolveId(source) {
    if (source.includes("types/tool")) {
      return path.resolve(import.meta.dirname, "shared", "const.ts");
    }
    return null;
  },
});
